...
- Often called DMA
- Is a feature of Microcontrollers that allows peripherals (ADC, DAC, SPI) to operate without needing CPU time
- Can interrupt CPU when done
...
## Communication
\dot(CodingCycle {width=100%})(Network Communication Coding Cycle)
~~~
digraph {
    rankdir=LR;
    "Source Coding" -> "Channel Coding" -> "Modulation" -> "Channel" -> "Demodulation" -> "Channel decoding" -> "Source decoding";
    "Noise" -> "Channel";

    {rank=same "Source Coding" "Source decoding"};
    {rank=same "Channel Coding" "Channel decoding"};
    {rank=same "Modulation" "Demodulation"};
}
~~~
...
- Modulation
    - e.g. Manchester
- Channel Coding
    - Error correct and whatever we can't correct we generate an error
    - e.g. Hamming, Parity Code, CRC
- Source Coding
    - Remove redundancy to reduce data size
    - e.g. Huffman

| **Letter** | **Value** | **Binary Result** |
|------------|-----------|-------------------|
| A          | 3         | 0101              |
| B          | 6         | 001               |
| C          | 8         | 10                |
...
\dot(HuffmanCode)(How To Generate Huffman Code)
~~~
digraph {
    "35" -> "15" [label="1"];
    "35" -> "20" [label="0"];
...
- Scheduling
- Memory Management
- Advanced FreeRTOS concepts (i.e. group events, etc)

> **Not Examined:** CLI, Embedded Networking, realtime messaging, multicore and IOT

### RTOS Fundamentals
    - Tasks, Synchronisation, Sharing, priorities, HAL, etc
    - Threads of execution (Tasks):
        - Small, schedulable and sequential programmable units
        - Concurrent operation and multitasking
    - Co-routine
        - Similar to a task but shares a stack amongst the co-routines
    - Synchronisation (Semaphores):
        - Allows tasks to be blocked or suspended until signalled
        - Provides mutual exclusion
        - Types: Binary, Counting, Mutex
    - Parameter Sharing (Queues):
        - Used to share variables between tasks
        - Allows synchronised reading/writing of items by multiple tasks -- i.e. Prevents multiple write errors
    - Software Timer:
        - Causes a function to execute after a period of time
### RTOS Kernel
    - Core of an RTOS
    - Performs Scheduling, Context Switching, Resource Management
        - Scheduling -- Detemines when a task can execute
        - Context Switching -- Allows tasks to be suspended and resumed without errors
        - Resource Management -- Memory allocation and de-allocation for RTOS elements
    - Memory Management:
        - Heap 1: Does not allow freeing of memory
        - Heap 2: Best Fit algorithm and allows memory to be freed
        - Heap 3: Implements Malloc and Free functions
...
### Interrupt Handling
- Each task has a Task Control Block (TCB) containing values of:
    - Microprocessor registers
    - Task State
    - Task Variables
    - Stack Variables
- If a task is to be interrupted or suspended, the TCB needs to be saved
- When the task is resumed, the TCB needs to be restored
- The TCB allows a task a suspended and resumed by the kernel
- Interrupt Service Routine must:
    1. Save the TCB
    1. Process the interrupt
        - Check for the correct interrupt conditions (i.e. mask flags)
        - Clear interrupt flags
    1. Perform any task synchronisation (i.e. trigger semaphore)
    1. Check if the interrupt handling task is of higher priority can be executed (context switch)
    1. Restore the TCB
    1. Kernel resumes execution
...
- Queue Set
    - A queue set is a collection of FreeRTOS queues and semaphores -- Similar to queues
    - Semaphores and queues can be added to a queue set
    - Queue sets provide a mechanism to allow an RTOS tasks to block (pend) on a read operation from multiple RTOS queues or semaphores simultaneously
    - A queue set can be used by a task to wait for an 'item' to be set -- i.e. a semaphore is taken or a queue receives an item
